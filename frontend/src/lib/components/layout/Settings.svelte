...
<div class="flex-1 p-8 max-w-4xl mx-auto w-full">
  ...
  <div class="space-y-8 pb-12">
    ...
    <section>
      ...
      <div
        class="bg-surface rounded-2xl border border-border p-5 shadow-sm space-y-6"
      >
        <div
          class="grid grid-cols-1 md:grid-cols-3 gap-3 items-start justify-items-center max-w-[600px] mx-auto"
        >
          <ThemePreviewButton
            value="dark"
            selectedValue={$theme}
    ...
    <section>
      ...
      <div
        class="bg-surface rounded-2xl border border-border p-5 flex items-center justify-between shadow-sm"
      >
        <div class="flex items-center gap-4">
          <div
            class="w-10 h-10 bg-accent/5 rounded-xl flex items-center justify-center text-accent"
          >
    ...
    <section class="pt-8 border-t border-border/50">
      ...
      >
        <div class="flex items-center gap-4 min-w-0">
          <div
            class="w-10 h-10 text-accent shrink-0 flex items-center justify-center"
          >
            {@html logoRaw}
          </div>
          <div class="min-w-0">
            <div
              class="font-bold text-accent tracking-tight font-display text-md truncate"
            >
        ...
        <div class="flex flex-col items-stretch sm:items-end gap-2 shrink-0">
          {#if $updateStore.updateAvailable}
            <p
              class="text-[11px] font-semibold text-amber-500/90 text-center sm:text-right"
            >
              {$t("updateAvailable").replace(
                "{version}",
          ...
          {:else}
            {#if $updateStore.status === "error"}
              <span
                class="text-[10px] font-semibold text-red-400/90 uppercase tracking-wide text-center sm:text-right"
              >
                {$t("updateCheckFailed")}
              </span>
            {:else if $updateStore.status === "ready"}
              <span
                class="text-[10px] font-bold text-emerald-500 uppercase tracking-widest text-center sm:text-right"
              >
                {$t("latestVersion")}
              </span>
            {/if}
