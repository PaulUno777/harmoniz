<script lang="ts">
  ...
  async function handleRename() {
    ...
    try {
      ...
      const newFilename =
        pathParts.length > 0
          ? (pathParts[pathParts.length - 1] ?? editFilename.trim())
          : editFilename.trim();
      const updated: Track = {
        ...selectedTrack,
        path: newPath,
  ...
  const isDirty = $derived(
    selectedTrack != null &&
      (editTitle !== (selectedTrack.title ?? "") ||
        editArtist !==
          (selectedTrack.artist ?? selectedTrack.artist_raw ?? "") ||
        editAlbum !== (selectedTrack.album ?? selectedTrack.album_raw ?? "") ||
        editYear !== (selectedTrack.year ?? 0) ||
        editTrackNum !== (selectedTrack.track_num ?? 0)),
  );
...
<aside class="w-80 h-full bg-surface border-l border-border flex flex-col pt-6">
  ...
  {#if selectedTrack}
    <div class="flex-1 overflow-y-auto px-6 space-y-5 pb-4">
      ...
      <div
        class="aspect-square w-full bg-background rounded-xl flex items-center justify-center border border-border group relative overflow-hidden"
      >
        <div
          class="flex flex-col items-center gap-2 text-text-secondary/40 transition-transform duration-300 group-hover:scale-110"
        >
      ...
      <div class="space-y-3">
        <!-- Title -->
        <div>
          <label for="ctx-title" class="field-label mb-1">{$t("title")}</label>
          <input
            id="ctx-title"
            type="text"
        ...
        <!-- Artist -->
        <div>
          <label for="ctx-artist" class="field-label mb-1">{$t("artist")}</label
          >
          <input
            id="ctx-artist"
            type="text"
        ...
        <!-- Album -->
        <div>
          <label for="ctx-album" class="field-label mb-1">{$t("album")}</label>
          <input
            id="ctx-album"
            type="text"
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label
              for="ctx-year"
              class="field-label mb-1 flex items-center gap-1"
            >
        ...
        <!-- Size (read-only) -->
        <div>
          <span class="field-label mb-1 flex items-center gap-1">
            <FileAudioIcon size={11} />{$t("size")}
          </span>
          <div
            class="px-3 py-1.5 text-xs text-text-secondary bg-background border border-border rounded-md opacity-60 select-all"
          >
        ...
        <div>
          ...
          <div
            class="text-[11px] font-mono break-all text-text-secondary/70 bg-background p-2 rounded border border-border leading-relaxed select-all"
          >
            {selectedTrack.path}
          </div>
        </div>
  ...
  {:else}
    <div
      class="flex-1 flex flex-col items-center justify-center text-text-secondary px-10 text-center gap-4"
    >
      <div
