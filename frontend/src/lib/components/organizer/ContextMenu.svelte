<script lang="ts">
  import { CheckIcon, XIcon, SpinnerIcon, FileAudioIcon } from 'phosphor-svelte'
...
>
  <!-- Header -->
  <div class="px-4 py-3 bg-background/60 border-b border-border flex items-center gap-2">
    <FileAudioIcon size={14} class="text-accent shrink-0" />
    <span class="text-xs font-mono text-text-secondary truncate flex-1" title={suggestion.track.filename}>
      {suggestion.track.filename}
    </span>
    <button on:click={onClose} class="text-text-muted hover:text-text-primary transition-colors shrink-0">
      <XIcon size={14} />
    </button>
  </div>
  ...
  <div class="px-4 py-2 border-b border-border/50 flex items-center gap-2">
    ...
    <span class="ml-auto text-xs font-bold tabular-nums {suggestion.score >= 75 ? 'text-green-400' : suggestion.score >= 50 ? 'text-yellow-400' : 'text-text-muted'}">
      ...
      {#each sortedFields as [key, field]}
        <div class="px-4 py-2.5">
          <div class="flex items-center justify-between mb-1">
            <span class="text-[10px] font-bold uppercase tracking-wider text-text-secondary/70">
              {fieldLabels[key] ?? key}
            </span>
            <div class="flex items-center gap-1.5">
              <span class="text-[9px] px-1.5 py-0.5 rounded-full bg-white/5 text-text-muted uppercase">
                {sourceLabel(field.source)}
              </span>
              <span class="text-[10px] font-bold tabular-nums {confidenceClass(field.confidence)}">
          ...
          <div class="flex items-center gap-2">
            <div class="flex-1 min-w-0">
              {#if currentValue(key)}
                <div class="text-[10px] text-text-muted truncate line-through opacity-50">
                  {currentValue(key)}
                </div>
              {/if}
