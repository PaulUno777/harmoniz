<script lang="ts">
  ...
  import {
    FloppyDiskIcon, XIcon, CheckCircleIcon, ArrowCounterClockwiseIcon,
    MusicNoteSimpleIcon, SpinnerIcon, CalendarIcon, HashIcon, FolderIcon,
  ...
  export let onApply: (trackID: number, fields: Record<string, string>) => Promise<void>
  ...
  function loadFromSuggestion(s: OrganizerSuggestion) {
    const tk = s.track
    const f  = s.fields
    suggestedFields = new Set(Object.keys(f))
    ...
    editTrackNum = f['track_num'] ? f['track_num'].value : (tk.track_num  ? String(tk.track_num) : '')
  }

  // Reactive diff — all local vars in args so Svelte 4 tracks them
  $: changedFields = computeChanges(suggestion, editArtist, editTitle, editAlbum, editYear, editTrackNum)
  $: hasChanges    = Object.keys(changedFields).length > 0

  function computeChanges(
    s: OrganizerSuggestion | null,
    artist: string, title: string, album: string, year: string, trackNum: string
  ...
  function revertField(key: string) {
    ...
    if (key === 'track_num') editTrackNum = tk.track_num  ? String(tk.track_num) : ''
  }
  ...
  const fieldLabels: Record<string, string> = {
    artist: 'Artist', title: 'Title', album: 'Album', year: 'Year', track_num: 'Track #'
  }
  ...
  $: suggestedFilename = computeFilename(
    suggestion, $filenameTemplate, editArtist, editTitle, editAlbum, editYear, editTrackNum
  )

  function computeFilename(
    s: OrganizerSuggestion | null, tmpl: string,
    artist: string, title: string, album: string, year: string, trackNum: string
  ): string {
    if (!s || !tmpl) return ''
...
{#if !suggestion}
  <div class="h-full flex flex-col items-center justify-center text-center px-10 gap-4">
...
{:else}
  <div class="h-full flex flex-col bg-surface border-l border-border">
    ...
    <div class="flex-1 overflow-y-auto px-6 py-4 space-y-4 min-h-0">
      ...
      <div class="aspect-square w-full bg-background rounded-xl flex flex-col items-center justify-center
        ...
        {#if Object.keys(suggestion.fields).length > 0}
          <div class="absolute bottom-0 left-0 right-0 px-3 pb-3 pt-6
            ...
            <div class="flex items-center justify-between mb-1.5">
              <span class="text-[9px] font-bold uppercase tracking-wider text-text-muted/50">Confidence</span>
              <span class="text-[10px] font-bold tabular-nums
                           {suggestion.score >= 75 ? 'text-green-400' : suggestion.score >= 50 ? 'text-yellow-400' : 'text-orange-400'}">
            ...
            <div class="h-1 bg-white/5 rounded-full overflow-hidden">
              <div
                class="h-full rounded-full transition-all
                       {suggestion.score >= 75 ? 'bg-green-500' : suggestion.score >= 50 ? 'bg-yellow-500' : 'bg-orange-500'}"
      ...
      <div class="text-center -mt-1 pb-1">
        <div class="text-sm font-bold text-text-primary truncate">
          {(suggestion.fields['title']?.value) || suggestion.track.title || suggestion.track.filename}
        </div>
        <div class="text-xs text-text-muted truncate mt-0.5">
          {(suggestion.fields['artist']?.value) || suggestion.track.artist_raw || '—'}
          {#if suggestion.track.album_raw || suggestion.fields['album']?.value}
            <span class="opacity-30 mx-1">·</span>{suggestion.fields['album']?.value || suggestion.track.album_raw}
          {/if}
        </div>
      </div>
      ...
      <div class="space-y-3">
        ...
        <div>
          <div class="flex items-center gap-2 mb-1">
            <label for="org-artist" class="field-label">{fieldLabels['artist']}</label>
            {#if artistField}
              <span class="text-[9px] px-1.5 py-0.5 rounded-full bg-white/5 text-text-muted uppercase leading-none">
                {artistField.source}
              </span>
              <span class="text-[9px] font-bold {confClass(artistField.confidence)}">
                {Math.round(artistField.confidence * 100)}%
              </span>
            {/if}
            {#if artistChanged}
              <button on:click={() => revertField('artist')}
          ...
          <input id="org-artist" type="text" bind:value={editArtist}
            class="field-input {artistChanged ? 'border-accent/50 ring-1 ring-accent/20' : ''}" />
        ...
        <div>
          <div class="flex items-center gap-2 mb-1">
            <label for="org-title" class="field-label">{fieldLabels['title']}</label>
            {#if titleField}
              <span class="text-[9px] px-1.5 py-0.5 rounded-full bg-white/5 text-text-muted uppercase leading-none">
                {titleField.source}
              </span>
              <span class="text-[9px] font-bold {confClass(titleField.confidence)}">
                {Math.round(titleField.confidence * 100)}%
              </span>
            {/if}
            {#if titleChanged}
              <button on:click={() => revertField('title')}
                class="ml-auto p-1 rounded hover:bg-white/5 text-text-muted/40 hover:text-text-muted transition-colors"
                title="Revert">
                <ArrowCounterClockwiseIcon size={11} />
              </button>
          ...
          <input id="org-title" type="text" bind:value={editTitle}
            class="field-input {titleChanged ? 'border-accent/50 ring-1 ring-accent/20' : ''}" />
        ...
        <div>
          <div class="flex items-center gap-2 mb-1">
            <label for="org-album" class="field-label">{fieldLabels['album']}</label>
            {#if albumField}
              <span class="text-[9px] px-1.5 py-0.5 rounded-full bg-white/5 text-text-muted uppercase leading-none">
                {albumField.source}
              </span>
              <span class="text-[9px] font-bold {confClass(albumField.confidence)}">
                {Math.round(albumField.confidence * 100)}%
              </span>
            {/if}
            {#if albumChanged}
              <button on:click={() => revertField('album')}
                class="ml-auto p-1 rounded hover:bg-white/5 text-text-muted/40 hover:text-text-muted transition-colors"
                title="Revert">
                <ArrowCounterClockwiseIcon size={11} />
              </button>
          ...
          <input id="org-album" type="text" bind:value={editAlbum}
            class="field-input {albumChanged ? 'border-accent/50 ring-1 ring-accent/20' : ''}" />
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div>
            <div class="flex items-center gap-1.5 mb-1">
              ...
              <label for="org-year" class="field-label">{fieldLabels['year']}</label>
              {#if yearField}
                <span class="text-[9px] font-bold {confClass(yearField.confidence)}">
                  {Math.round(yearField.confidence * 100)}%
                </span>
              {/if}
            </div>
            <input id="org-year" type="number" bind:value={editYear} min="0" max="9999"
              placeholder="0"
              class="field-input {yearChanged ? 'border-accent/50 ring-1 ring-accent/20' : ''}" />
          </div>
          ...
          <div>
            <div class="flex items-center gap-1.5 mb-1">
              ...
              <label for="org-tracknum" class="field-label">{fieldLabels['track_num']}</label>
              {#if trackField}
                <span class="text-[9px] font-bold {confClass(trackField.confidence)}">
                  {Math.round(trackField.confidence * 100)}%
                </span>
              {/if}
            </div>
            <input id="org-tracknum" type="number" bind:value={editTrackNum} min="0"
              placeholder="0"
              class="field-input {trackChanged ? 'border-accent/50 ring-1 ring-accent/20' : ''}" />
          </div>
        ...
        <div>
          ...
          <div class="px-3 py-1.5 text-[11px] font-mono text-text-muted/50
        ...
        {#if suggestedFilename && suggestedFilename !== suggestion.track.filename}
          <div>
            ...
            <div class="px-3 py-1.5 text-[11px] font-mono text-accent/90
                        bg-accent/5 border border-accent/20 rounded-md break-all select-all leading-relaxed">
              {suggestedFilename}
            </div>
            {#if $filenameTemplate.includes('/')}
              <p class="text-[9px] text-text-muted/40 mt-1">File will be moved into this subfolder on save</p>
            {/if}
          </div>
        {/if}
        ...
        {#if Object.keys(suggestion.fields).length === 0}
          <div class="flex flex-col items-center gap-2 py-4 text-center">
            <CheckCircleIcon size={28} weight="thin" class="text-green-400/60" />
            <p class="text-xs text-green-400/70 font-medium">Metadata looks good</p>
            <p class="text-[10px] text-text-muted/40">You can still edit fields manually above</p>
          </div>
        {/if}
    ...
    <div class="shrink-0 p-5 border-t border-border">
      ...
      {#if pendingConfirm}
        ...
        <div class="mb-3 rounded-xl bg-background border border-border/50 p-3 space-y-1.5">
          <p class="text-xs font-bold text-text-primary">
            Save {Object.keys(changedFields).length} change{Object.keys(changedFields).length !== 1 ? 's' : ''}?
          </p>
          {#each Object.entries(changedFields) as [key, ch]}
            <div class="flex items-center gap-2 text-[10px] text-text-muted">
              <span class="font-bold text-text-secondary uppercase w-12 shrink-0">{fieldLabels[key] ?? key}</span>
              <span class="line-through opacity-40 truncate max-w-[70px]">{ch.from || '(empty)'}</span>
              <span class="opacity-30 shrink-0">→</span>
              <span class="text-text-primary font-semibold truncate">{ch.to || '(empty)'}</span>
            </div>
          {/each}
        </div>
