<script lang="ts">
  ...
  import {
    MagicWandIcon, ArrowClockwiseIcon, FolderIcon, SpinnerIcon,
  ...
  async function handlePreviewTemplate() {
    ...
    try {
      const previews: FilenamePreview[] = await PreviewFilenameTemplate(libraryPath, get(filenameTemplate))
...
<div class="relative h-full flex flex-col min-h-0 overflow-hidden">
  ...
  <div class="shrink-0 px-5 pt-4 pb-3 border-b border-border space-y-2.5">
    ...
    <div class="flex items-center gap-2">
      <label for="org-tmpl" class="text-[10px] font-bold uppercase tracking-wider text-text-muted/70 shrink-0">
        {tr('filenameTemplate')}
      </label>
      <input
        id="org-tmpl"
    ...
    <p class="text-[10px] text-text-muted/40 font-mono">{tr('templatePlaceholders')}</p>
  </div>

  <!-- ───── Progress bar ───── -->
  {#if $status === 'analyzing'}
    <div class="shrink-0 h-0.5 bg-background overflow-hidden">
      <div class="h-full bg-accent" style="animation: indeterminate 1.5s ease-in-out infinite alternate"></div>
    </div>
  {/if}
  ...
  <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
    ...
    {#if $status === 'ready' && $suggestions.length > 0}
      <div class="shrink-0 px-4 py-2 flex items-center gap-2 border-b border-border/50">
        <span class="text-[10px] text-text-muted">
          <span class="font-bold text-text-primary">{$withSuggestions.length}</span>
          with suggestions
          {#if $suggestions.length !== $withSuggestions.length}
            · {$suggestions.length} total
          {/if}
        </span>
        <div class="ml-auto flex rounded-md overflow-hidden border border-border text-[10px] font-bold">
          <button
          ...
          <button
            ...
            class="px-2 py-1 border-l border-border transition-colors {showAll ? 'bg-accent/20 text-accent' : 'text-text-muted hover:bg-white/5'}"
          >
            All
          </button>
          <button
    ...
    <div class="flex-1 overflow-y-auto min-h-0">
      ...
      {#if !libraryPath}
        <div class="flex flex-col items-center justify-center h-full gap-4 text-center px-8">
          ...
          <div>
            <p class="text-sm text-text-secondary font-medium mb-1">{tr('noLibraryForOrganizer')}</p>
      ...
      {:else if $status === 'idle'}
        <div class="flex flex-col items-center justify-center h-full gap-4 text-center px-8">
          ...
          <div>
            <p class="text-sm text-text-secondary font-medium">{tr('organizerEmptyHint')}</p>
            <p class="text-[10px] text-text-muted/50 font-mono mt-1 truncate max-w-xs">{libraryPath}</p>
          </div>
          <button
            on:click={handleAnalyze}
      ...
      <div class="bg-surface border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden">
        <div class="px-5 py-4 border-b border-border flex items-center justify-between">
          <div>
            ...
            <p class="text-xs text-text-muted mt-0.5 font-mono">{$filenameTemplate}</p>
          </div>
          <button
        ...
        <div class="flex-1 overflow-y-auto px-5 py-3 space-y-1.5">
          ...
          {:else}
            ...
            {#if $filenamePreviews.length > 200}
              <p class="text-xs text-text-muted text-center pt-2">…and {$filenamePreviews.length - 200} more</p>
            {/if}
          {/if}
        </div>
